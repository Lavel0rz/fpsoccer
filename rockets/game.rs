...
impl Game {
    ...
    pub fn update(&mut self, fixed_dt: f32, game_width: f32, game_height: f32) {
        ...
        for player in self.players.values_mut() {
            ...
            // Handle projectile firing with boost button
            if player.input.boost && player.shoot_cooldown <= 0.0 {
                // Get aim direction from target coordinates
                if let (Some(target_x), Some(target_y)) = (player.input.target_x, player.input.target_y) {
                    ...
                    if mag > 0.0 {
                        ...
                        // Add player's velocity to projectile (like the ball shooting)
