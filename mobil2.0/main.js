...
class MainScene extends Phaser.Scene {
  ...
  setupMobileControls() {
    ...
    // Create a virtual joystick for movement
    ...
    // Add touch screen shooting (like left click on desktop)
    // Create an invisible touch area that covers the center of the screen
    // (avoiding the joystick and button areas)
    this.touchArea = this.add.rectangle(
      width / 2,
      height / 2,
      width - 200, // Leave space for controls on edges
      height - 200,
      0xffffff,
      0 // Fully transparent
    );
    ...
    // Add touch shooting only to the touch area
    this.touchArea.on('pointerdown', () => {
      console.log('Screen touched - shooting');
      this.inputState.shoot = true;
      this.sendInput();
  ...
  sendInput() {
    ...
    if (this.playerCanMove !== false && this.socket && this.socket.readyState === WebSocket.OPEN) {
      ...
      const targetX = this.ship.x + this.movementDirection.x * aimDistance;
      const targetY = this.ship.y + this.movementDirection.y * aimDistance;
      ...
      if (this.isMobile && this.joystick && this.joystick.force > 0) {
        input.joystick = {
    ...
    if (this.playerCanMove !== false) {
      ...
      if (this.isMobile && this.joystick) {
        ...
        if (this.joystick.force > 0) {
          this.targetDirection.x = this.joystick.forceX;
          this.targetDirection.y = this.joystick.forceY;
          ...
          // Convert joystick direction to input state for server communication
          ...
          if (this.targetDirection.x < -0.3) this.inputState.left = true;
          if (this.targetDirection.x > 0.3) this.inputState.right = true;
          if (this.targetDirection.y < -0.3) this.inputState.up = true;
          if (this.targetDirection.y > 0.3) this.inputState.down = true;
          
          // Send input to server when joystick changes significantly
          this.sendInput();
        } else {
          // If joystick is released, reset all directions
    ...
    if (this.isMobile) {
      try {
        if (this.joystick && this.joystick.base && this.joystick.thumb) {
          ...
          this.joystick.setPosition(100, gameSize.height - 100);
        }
        
        if (this.shootButton) {
          // Position shoot button at bottom right
          this.shootButton.x = gameSize.width - 80;
          this.shootButton.y = gameSize.height - 80;
          ...
          if (shootText) {
            shootText.x = gameSize.width - 80;
            shootText.y = gameSize.height - 80;
          }
        }
        
        if (this.boostButton) {
          // Position boost button above shoot button
          this.boostButton.x = gameSize.width - 80;
          this.boostButton.y = gameSize.height - 160;
          
          // Find and update the boost button text
          const boostText = this.children.list.find(child => 
            child.type === 'Text' && child.text === 'BOOST'
          );
          if (boostText) {
            boostText.x = gameSize.width - 80;
            boostText.y = gameSize.height - 160;
          }
        }
      } catch (error) {
  ...
  updateControllerInput() {
    ...
    for (const gamepad of gamepads) {
      ...
      // Face buttons (A/B/X/Y) for shooting - buttons 0-3
      // Using any of these buttons will trigger shooting
      const shootPressed = 
        gamepad.buttons[0].pressed || // A/Cross
        gamepad.buttons[1].pressed || // B/Circle
      ...
      // Shoulder buttons/triggers for boost (buttons 4-7)
      this.inputState.boost = 
        gamepad.buttons[4].pressed || // LB/L1
        gamepad.buttons[5].pressed || // RB/R1
        gamepad.buttons[6].pressed || // LT/L2
        gamepad.buttons[7].pressed;   // RT/R2
