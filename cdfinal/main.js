...
class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.currentMapName = 'cornerdefense'; // Since we're loading cornerdefense.json
  ...
  update(time, delta) {
    ...
    // For cornerdefense auto-shooting, continuously send input to server
    // This ensures server always has current mouse target position for auto-shooting
    if (this.isCornerDefenseMap && this.useServerAutoShooting && this.ship) {
      // Send input every frame to keep server updated with mouse position
      this.sendInput();
    }
